...
``` r
requireEunomia()
#> ℹ `EUNOMIA_DATA_FOLDER` set to: /tmp/Rtmp9xRU1D.
#> 
#> Download completed!
db <- dbConnect(duckdb(), dbdir = eunomiaDir())
#> Creating CDM database /tmp/Rtmp9xRU1D/GiBleed_5.3.zip
cdm <- cdmFromCon(db, 
                  cdmSchema = "main", 
                  writeSchema = "main", 
                  writePrefix = "cg_")
...
#> Rows: ??
#> Columns: 10
#> Database: DuckDB 1.4.3 [unknown@Linux 6.11.0-1018-azure:R 4.5.2//tmp/Rtmp9xRU1D/file227654f6e57b.duckdb]
